...
int ATParser::vscanf(const char *format, va_list args)
{
    ...
    while (true) {
        ...
        if ((count == j) && (_buffer[offset - 4] == _buffer[offset + j - 1])) {
...
bool ATParser::vsend(bool newline, const char *command, va_list args)
{
...
bool ATParser::send(const char *command, ...)
{
    ...
    bool res = vsend(true, command, args);
    va_end(args);
    return res;
}

bool ATParser::sendNoNewline(const char *command, ...)
{
    va_list args;
    va_start(args, command);
    bool res = vsend(false, command, args);
    va_end(args);
    return res;
}
